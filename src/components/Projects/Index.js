            ...
            <div className={`${classes.projectsContainer} ${classes[theme]}`}>
                {projectsData.map((project, index) => (
                    <div key={index} className={classes.projects}>
                        <img src={process.env.PUBLIC_URL + project.projectIcon} alt={`${project.projectName} icon`} className={classes.icon} />
                        <p className={classes.project}>{project.projectName}</p>
                        <a className={classes.projectLink} href={project.projectLink} target="_blank" rel="noopener noreferrer">
                            {project.projectLink}
                        </a>
                    </div>
                ))}
